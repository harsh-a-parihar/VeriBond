...
export default function LandingPage() {
  ...
  const tickerData = [
    { s: 'AGENT_ALPHA', t: 98.2, st: 'up' },
    { s: 'YIELD_SCOUT', t: 99.1, st: 'up' },
    { s: 'RISK_DAO', t: 82.4, st: 'down' },
    { s: 'MEV_GUARD', t: 94.5, st: 'up' },
    { s: 'PREDICT_ETH', t: 45.0, st: 'down' },
    { s: 'ARB_BOT_X', t: 91.2, st: 'up' },
  ];
      ...
      <div className="fixed top-0 w-full h-8 bg-[#020202] border-b border-white/5 z-[60] overflow-hidden flex items-center">
        <div className="animate-marquee flex items-center h-full whitespace-nowrap">
          {[...tickerData, ...tickerData, ...tickerData].map((d, i) => (
            <TickerItem key={i} symbol={d.s} trust={d.t} status={d.st as 'up' | 'down'} />
          ))}
        </div>
      </div>
      ...
      <main className="relative z-10 pt-40 pb-20">
        ...
        <section id="protocol" className="py-32 px-6 lg:px-12 max-w-[1600px] mx-auto relative">
          <div className="mb-20 md:max-w-2xl">
            ...
            <p className="text-xl text-zinc-400 font-light">
              ...
              into one verifiable lifecycle for AI agents.
            </p>
          </div>
        ...
        <section id="tracks" className="py-28 px-6 lg:px-12 max-w-[1600px] mx-auto">
          <div className="mb-10 md:max-w-2xl">
            ...
            <h2 className="text-4xl font-medium text-white mb-6">Infrastructure and tracks used in this build.</h2>
            <p className="text-lg text-zinc-400">The protocol combines multiple ecosystem tracks into one user-facing lifecycle.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            <TrackCard
              title="Base"
              detail="Contracts, auction flow, and app state run on Base Sepolia for public testnet validation."
            />
            <TrackCard
              title="Continuous Clearing Auction"
              detail="Token discovery and distribution happen through CCA launch mechanics."
            />
            <TrackCard
              title="Uniswap v4"
              detail="Post-auction liquidity is seeded into v4 so users can trade agent tokens."
            />
            <TrackCard
            ...
            <TrackCard
              ...
              detail="On-chain identity and verifiable names anchor persistent agent reputation."
            />
            <TrackCard
              title="Verifiable Event Indexing"
              detail="Indexer mirrors contract events into queryable app state for marketplace and analytics."
            />
          </div>
        </section>
